...
interface DialogProps {
  ...
  className?: string;
  // Extra classes for header, title and body sections
...
const Dialog: React.FC<DialogProps> = ({
  ...
  return createPortal(
    ...
    >
      ...
      >
        {title != null && title !== '' && (
          <div className={`flex items-center mb-2 p-4 pb-0 ${headerClassName}`.trim()}>
            <h2 className={`title p-1 flex-1 ${titleClassName}`.trim()} data-test-id="dialog-title">
              {title}
            </h2>
            {showCloseButton && (
