...
const InventoryBookReport: React.FC<InventoryBookReportProps> = ({
  ...
  const handleExportExcel = useCallback(async () => {
    if (!filters.seasonId || !filters.month) {
      setExportError('Debe seleccionar temporada y mes para exportar.');
      return;
    }
    ...
    try {
      ...
      if (!result.success || !result.data) {
        setExportError(result.error || 'No fue posible exportar el Libro de Existencias a Excel.');
        return;
      }
      ...
      link.download = result.fileName || 'libro-existencias.xlsx';
      ...
      setExportError(
        error instanceof Error
          ? error.message
          : 'No fue posible exportar el Libro de Existencias a Excel.',
      );
    } finally {
    ...
    <div className="flex flex-col gap-6 p-6 print:p-0 print:gap-4">
      ...
      <div className="flex justify-end gap-2 print:hidden">
        ...
        <IconButton
          icon="file_download"
          variant="basicSecondary"
          disabled={!report || loading || isExporting}
      ...
      {exportError && <Alert variant="error">Error al exportar: {exportError}</Alert>}
