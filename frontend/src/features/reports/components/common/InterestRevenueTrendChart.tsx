...
import { FinancialServicesTrendItem, ReportGroupBy } from '../../types/reports.types';
import { REPORT_GROUP_BY_LABELS } from './reportUtils';

interface InterestRevenueTrendChartProps {
...
const InterestRevenueTrendChart: React.FC<InterestRevenueTrendChartProps> = ({
  trend,
  groupBy,
  currencyFormatter,
}) => {
  ...
  const chartData = useMemo<LineSeries[]>(
    () => [
      {
        id: 'Capital Colocado',
        data: trend.map((item) => ({ x: item.period, y: item.capitalPlaced })),
      },
      {
        id: 'Interés Neto',
        data: trend.map((item) => ({ x: item.period, y: item.accruedInterestNet })),
      },
      {
        id: 'IVA Estimado',
        data: trend.map((item) => ({ x: item.period, y: item.estimatedInterestVat })),
      },
      {
        id: 'Total Financiero',
        data: trend.map((item) => ({ x: item.period, y: item.totalFinancialRevenue })),
      },
    ],
    [trend],
  );
  ...
  return (
    <section className="rounded-xl border border-neutral-200 bg-white px-2 py-3 print:hidden">
      <div className="border-b border-neutral-200 px-2 py-1">
        <h3 className="text-base font-semibold text-neutral-900">Gráfico de Tendencia temporal</h3>
        <p className="mt-1 text-sm text-neutral-600">
          Evolución de capital, interés neto, IVA y total según agrupación por {REPORT_GROUP_BY_LABELS[groupBy].toLowerCase()}.
        </p>
      </div>
      ...
      <div className="h-[380px] w-full pt-3">
        <ResponsiveLine
          ...
          enableGridX={false}
          colors={['#475569', '#0f766e', '#0ea5e9', '#0f172a']}
          pointSize={8}
...
export default InterestRevenueTrendChart;
