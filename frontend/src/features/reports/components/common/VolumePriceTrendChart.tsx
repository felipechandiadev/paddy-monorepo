...
import { VolumePriceMonthlyItem } from '../../types/reports.types';

interface VolumePriceTrendChartProps {
...
const VolumePriceTrendChart: React.FC<VolumePriceTrendChartProps> = ({
  monthly,
  numberFormatter,
}) => {
  ...
  const chartData = useMemo<LineSeries[]>(
    () => [
      {
        id: 'Kg Bruto',
        data: monthly.map((item) => ({ x: item.month, y: item.grossKg })),
      },
      {
        id: 'Kg Neto',
        data: monthly.map((item) => ({ x: item.month, y: item.netKg })),
      },
    ],
    [monthly],
  );
  ...
  if (monthly.length === 0) {
    return (
      <section className="rounded-xl border border-neutral-200 bg-white px-4 py-4 print:hidden">
        <h3 className="text-base font-semibold text-neutral-900">Gráfico de Volumen mensual</h3>
  ...
  return (
    <section className="rounded-xl border border-neutral-200 bg-white px-2 py-3 print:hidden">
      <div className="border-b border-neutral-200 px-2 py-1">
        <h3 className="text-base font-semibold text-neutral-900">Gráfico de Volumen mensual</h3>
        <p className="mt-1 text-sm text-neutral-600">
          Evolución comparada entre kilos brutos y netos comprados por mes.
        </p>
      </div>
      ...
      <div className="h-[360px] w-full pt-3">
        <ResponsiveLine
          data={chartData}
          margin={{ top: 20, right: 20, bottom: 96, left: 96 }}
          xScale={{ type: 'point' }}
...
export default VolumePriceTrendChart;
