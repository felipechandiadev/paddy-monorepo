...
import { createRiceType, fetchRiceTypes } from '../actions';
import { RiceType } from '../types/rice-types.types';

interface CreateRiceTypeDialogProps {
  open: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export default function CreateRiceTypeDialog({
  open,
  onClose,
  onSuccess,
}: CreateRiceTypeDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        setFormData({ name: '', code: '', description: '', referencePrice: 0, isActive: true });
        onClose();
        onSuccess();
      } else {
        setError(result.error || 'Error al crear tipo de arroz');
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Error al crear tipo de arroz');
    } finally {
      setLoading(false);
    }
  };
