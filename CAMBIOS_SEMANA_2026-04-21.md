...
**Periodo considerado:** 21 al 24 de abril de 2026 (fecha de autor del commit).  
**Total de commits:** 89.  
**Última actualización de este archivo:** 24/04/2026 (commit de documentación el más reciente en `git log`; incluye cambios posteriores a `67440b3`).

Este resumen se elaboró a partir de los mensajes de commit en el repositorio. Incluye además notas de operación en entorno local que no siempre tienen commit asociado.
...
### Commits posteriores a `67440b3` (23/04)

- **`d3826e0` — feat: cargo TMS, deploy, rol TRUCK_RECEPTION y ajustes de UI**  
  - Renombrado / consolidación **paddy-tms → cargo** (app Next de báscula/TMS), con **deploy** (PM2, `deploy/README`, secrets de ejemplo).  
  - **Rol `TRUCK_RECEPTION`:** migración en `database/migrations`, mensajes de error al persistir; frontend con tipo `UserRole`, badges y etiqueta «Recepción y desapacho de carga».  
  - **Cargo (UI):** placeholders retirados en formularios de recepción/despacho; diálogo de turno con filas ocupadas resaltadas; ticket de impresión con chofer/empresa solo si hay datos; puerto serial con `Select` compartido; DataGrids (columna acciones sin título); lista despachos sin título/enlace extra; edición recepción con pesos enteros.  
  - Varios ajustes en frontend principal (login, informes) según el mismo commit.

- **`86acd64` — chore: gitignore informe y changelogs; limpieza backend y artefactos**  
  - **`.gitignore`:** carpeta `/informe/`, `/CAMBIOS_SEMANA_*.md`; deja de versionarse el gitlink de **informe** y el changelog semanal en el índice.  
  - **Backend:** eliminación de documentación `.md` duplicada (se conserva `README.md`), scripts **backfill** y otros utilitarios sueltos; **`package-lock.json`** en backend/frontend/cargo; referencias en Copilot y comentario en `seed-test-cosecha`.  
  - Limpieza de documentación local en **cargo** y ajustes menores en caché **turbopack** / restos **paddy-tms**.

- **`0e2a8d3` — fix(backend): restaurar `db-reset.ts`**  
  - Vuelve el archivo **`backend/db-reset.ts`** necesario para `npm run db:reset` y `db:setup` tras la limpieza anterior.

- **`79f4a88` — chore: bump version to 1.2.6 (backend + frontend)**  
  - Alineación de versión en **`backend/package.json`** y **`frontend/package.json`** (`1.2.5` → `1.2.6`).

- **docs: actualizar CAMBIOS_SEMANA_2026-04-21 (v1.2.6 y notas DB local)**  
  - Este archivo pasa a versionarse en Git (`git add -f`, pese al patrón en `.gitignore`); resume el bump **1.2.6** y el flujo de respaldo / restore de MySQL local.

### Entorno local / base de datos (23–24 abr., operación sin commit en repo)

- **Respaldo previo** antes de tocar datos: `mysqldump` de la base `paddy` local (archivo bajo `backend/backups/`, carpeta ignorada por Git).
- **Sincronización con datos de producción (dump `paddy_20260424_022601.sql`):** import completo del SQL y luego **`npm run db:migrate`** para aplicar migraciones TMS no presentes en prod; tablas **`truck_receptions`** y **`truck_dispatches`** quedan vacías; resto de tablas con datos del dump.
- **Nota técnica:** ejecutar solo **`db:migrate`** sobre una base **vacía** falla, porque las migraciones en `backend/src/infrastructure/database/migrations/` asumen tablas base ya creadas; el flujo práctico fue **dump + migraciones**.
- **Respaldo del estado local “actual”** tras esa operación: de nuevo `mysqldump` (p. ej. `paddy_local_20260423_231427.sql` en `backend/backups/`) para clonar o restaurar el entorno con esquema y **`migrations`** al día.

---

## Lista completa de commits (orden cronológico inverso: más reciente primero)

- docs: actualizar CAMBIOS_SEMANA_2026-04-21 (v1.2.6 y notas DB local) (2026-04-24)
- `79f4a88` — chore: bump version to 1.2.6 (backend + frontend) (2026-04-24)
- `0e2a8d3` — fix(backend): restaurar db-reset.ts requerido por npm run db:reset / db:setup (2026-04-23)
- `86acd64` — chore: gitignore informe y changelogs; limpieza backend y artefactos (2026-04-23)
- `d3826e0` — feat: cargo TMS, deploy, rol TRUCK_RECEPTION y ajustes de UI (2026-04-23)
